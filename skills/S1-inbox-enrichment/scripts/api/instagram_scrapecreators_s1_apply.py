...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply Instagram ScrapeCreators export back to S1 existing columns only.")
    parser.add_argument("--csv", required=True, help="Target S1 CSV")
    parser.add_argument("--json", required=True, help="Raw JSON exported by instagram_scrapecreators_s1_export.py")
...
def clean_author_name(value: object, handle: object = "", blocked_tokens: list[str] | None = None) -> str:
    ...
    if "@" in text or re.search(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", text, re.I):
        return ""
    if any(token in lower for token in ["followers", "following", "posts"]):
        return ""
...
def pick_identity(payload: dict, platform: str, current_handle: object, current_name: object, url: object) -> tuple[str, str]:
    ...
    api_handle = normalize_handle(user.get("username"))
    api_name = clean_author_name(user.get("full_name"), api_handle or current_handle, ["instagram"])
    url_handle = extract_handle_from_url(platform, url)
    best_handle = api_handle or url_handle or normalize_handle(current_handle)
    best_name = api_name or clean_author_name(current_name, best_handle, ["instagram"])
...
def infer_language(payload: dict) -> tuple[str, str]:
    ...
    source = " ".join(
        [
            clean_text(user.get("biography")),
            clean_text(user.get("full_name")),
            clean_text(user.get("category_name")),
        ]
    ).lower()
    if not source:
        return "", ""
    if re.search(r"[\u4e00-\u9fff]", source):
        return "中文", "biography/full_name 出现中文"
    if re.search(r"[\u3040-\u30ff]", source):
        return "日语", "biography/full_name 出现日文"
    if re.search(r"[\u0600-\u06ff]", source):
        return "阿拉伯语", "biography/full_name 出现阿文"
    if any(token in source for token in ["bonjour", "france", "français", "francais", "paris"]):
        return "法语", "biography 出现法语信号"
    if any(token in source for token in ["hola", "españ", "espan", "latino"]):
        return "西班牙语", "biography 出现西语信号"
    if any(token in source for token in ["italia", "italy", "ciao"]):
        return "意大利语", "biography 出现意大利语信号"
    return "英语", "biography 默认英语信号"


def infer_country(payload: dict) -> tuple[str, str]:
    user = payload.get("data", {}).get("user", {})
    text = " ".join([clean_text(user.get("biography")), clean_text(user.get("full_name"))]).lower()
    padded = f" {text} "
    rules = [
        ("英国", [" uk ", "london", "british", "england", "🇬🇧"]),
    ...
    for country, signals in rules:
        if any(signal in padded or signal in text for signal in signals):
            return country, f"biography/full_name 命中 {country} 信号"
    return "", ""
...
def is_usable_payload(item: dict) -> bool:
    ...
    if payload.get("accountDoesNotExist") or payload.get("errorStatus") == 404:
        return False
    return isinstance(user, dict) and bool(user.get("username") or user.get("full_name"))
...
def extract_email(payload: dict) -> tuple[str, str]:
    user = payload.get("data", {}).get("user", {})
    email = clean_text(user.get("public_email"))
    if email:
        return email, "ScrapeCreators API 直出邮箱"
    biography = clean_text(user.get("biography"))
    match = re.search(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", biography, re.I)
    if match:
        return match.group(0), "ScrapeCreators API biography 正则提取"
    return "", ""
...
def main() -> None:
    ...
    for item in report.get("items", []):
        ...
        if best_handle and (not is_standard_handle(df.at[idx, "账号ID"]) or clean_text(df.at[idx, "账号ID"]) != best_handle):
        ...
        biography = clean_text(user.get("biography"))
        if biography and clean_text(df.at[idx, "账号简介__平台抓取"]) != biography:
            row_changes["账号简介__平台抓取"] = {"from": clean_text(df.at[idx, "账号简介__平台抓取"])[:120], "to": biography[:120]}
            df.at[idx, "账号简介__平台抓取"] = biography
            stats["bio_platform_updated"] += 1

        category = clean_text(user.get("category_name"))
        if category and clean_text(df.at[idx, "账号类目标签__平台抓取"]) != category:
            row_changes["账号类目标签__平台抓取"] = {"from": clean_text(df.at[idx, "账号类目标签__平台抓取"]), "to": category}
            df.at[idx, "账号类目标签__平台抓取"] = category
            stats["tags_platform_updated"] += 1

        merged_links = merge_links(df.at[idx, "外链__平台抓取"], extract_links(payload))
        if merged_links and clean_text(df.at[idx, "外链__平台抓取"]) != merged_links:
            row_changes["外链__平台抓取"] = {"from": clean_text(df.at[idx, "外链__平台抓取"]), "to": merged_links}
        ...
        if cross_platforms and clean_text(df.at[idx, "多平台标记"]) != cross_platforms:
            row_changes["多平台标记"] = {"from": clean_text(df.at[idx, "多平台标记"]), "to": cross_platforms}
            df.at[idx, "多平台标记"] = cross_platforms
            stats["cross_platform_updated"] += 1

        email, note = extract_email(payload)
        ...
        changes.append({"row": row_no, "handle": clean_text(df.at[idx, "账号ID"]), "url": clean_text(df.at[idx, "账号链接"]), "changes": row_changes})
    ...
    print(json.dumps({"timestamp": datetime.now().isoformat(timespec="seconds"), "csv_path": str(csv_path), "json_path": str(json_path), "mode": args.mode, "backup_path": backup, "stats": stats, "changes": changes}, ensure_ascii=False, indent=2))
