...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply TikTok ScrapeCreators export back to S1 existing columns only.")
    parser.add_argument("--csv", required=True, help="Target S1 CSV")
    parser.add_argument("--json", required=True, help="Raw JSON exported by tiktok_scrapecreators_s1_export.py")
...
def clean_author_name(value: object, handle: object = "", blocked_tokens: list[str] | None = None) -> str:
    ...
    if "@" in text or re.search(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", text, re.I):
        return ""
    if any(token in lower for token in ["followers", "following", "posts", "likes", "videos"]):
        return ""
...
def pick_identity(payload: dict, platform: str, current_handle: object, current_name: object, url: object) -> tuple[str, str]:
    ...
    api_handle = normalize_handle(user.get("uniqueId") or user.get("username"))
    api_name = clean_author_name(user.get("nickname"), api_handle or current_handle, ["tiktok"])
    url_handle = extract_handle_from_url(platform, url)
    best_handle = api_handle or url_handle or normalize_handle(current_handle)
    best_name = api_name or clean_author_name(current_name, best_handle, ["tiktok"])
...
def normalize_followers(payload: dict) -> str:
    ...
    user = payload.get("user") or {}
    count = stats.get("followerCount") or user.get("followerCount")
...
def infer_language(payload: dict) -> tuple[str, str]:
    ...
    source = " ".join(
        [
            clean_text(user.get("signature")),
            clean_text(user.get("nickname")),
            clean_text(json.dumps(user.get("bioLink") or {}, ensure_ascii=False)),
            clean_text(json.dumps(user.get("bioLinks") or [], ensure_ascii=False)),
        ]
    ).lower()
    if not source:
        return "", ""
    if re.search(r"[\u4e00-\u9fff]", source):
        return "中文", "signature/bioLink 出现中文"
    if re.search(r"[\u3040-\u30ff]", source):
        return "日语", "signature/bioLink 出现日文"
    if re.search(r"[\u0600-\u06ff]", source):
        return "阿拉伯语", "signature/bioLink 出现阿文"
    if any(token in source for token in ["bonjour", "france", "français", "francais"]):
        return "法语", "signature 出现法语信号"
    if any(token in source for token in ["hola", "españ", "espan", "latino"]):
        return "西班牙语", "signature 出现西语信号"
    return "英语", "signature 默认英语信号"


def infer_country(payload: dict) -> tuple[str, str]:
    user = payload.get("user") or {}
    text = " ".join([clean_text(user.get("signature")), clean_text(user.get("nickname"))]).lower()
    padded = f" {text} "
    rules = [
    ...
    for country, signals in rules:
        if any(signal in padded or signal in text for signal in signals):
            return country, f"signature/nickname 命中 {country} 信号"
    return "", ""
...
def is_usable_payload(item: dict) -> bool:
    ...
    if not payload.get("success"):
        return False
    user = payload.get("user") or {}
    return isinstance(user, dict) and bool(user.get("uniqueId") or user.get("nickname") or payload.get("stats"))
...
def extract_links(payload: dict) -> list[str]:
    ...
    if isinstance(bio_link, dict):
        link = clean_text(bio_link.get("link") or bio_link.get("url"))
        if link:
            links.append(link)
    for item in user.get("bioLinks") or []:
...
def extract_email(payload: dict) -> tuple[str, str]:
    ...
    signature = clean_text(user.get("signature"))
    match = re.search(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", signature, re.I)
    if match:
        return match.group(0), "ScrapeCreators API signature 正则提取"
    return "", ""
...
def main() -> None:
    ...
    for item in report.get("items", []):
        ...
        signature = clean_text(user.get("signature"))
        if signature and clean_text(df.at[idx, "账号简介__平台抓取"]) != signature:
            row_changes["账号简介__平台抓取"] = {
                "from": clean_text(df.at[idx, "账号简介__平台抓取"])[:120],
                "to": signature[:120],
            }
            df.at[idx, "账号简介__平台抓取"] = signature
