...
BASE_URL = "https://api.scrapecreators.com/v1/tiktok/profile"
PLATFORM = "TikTok"


def build_candidate_artifact(rows, model_results, validation):
    """Thin adapter to the shared S1 candidate contract (lazy import)."""
    try:
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export TikTok profile data from ScrapeCreators API into workbench JSON.")
    ...
    parser.add_argument("--limit", type=int, default=20, help="Max TikTok rows when --rows is empty")
    parser.add_argument("--date", default=datetime.now().strftime("%Y-%m-%d"), help="Workbench date folder")
    parser.add_argument("--prefix", default="tiktok_scrapecreators_batch", help="Output filename prefix")
...
def extract_handle(url: str, fallback_handle: str) -> str:
    ...
    handle = clean_text(fallback_handle).lstrip("@").strip(" ，,")
    return handle
...
def is_usable_payload(http_status: int, payload: object) -> bool:
    ...
    if not isinstance(user, dict):
        return False
    if not (user.get("uniqueId") or user.get("nickname") or payload.get("stats")):
        return False
    return True
    ...
    for row_no, row in rows:
        ...
        try:
            ...
            summary_items.append(
                {"row": row_no, "url": url, "status": "request_error", "reason": str(exc), "before": before}
            )
            continue

        user = payload.get("user") if isinstance(payload, dict) else {}
        stats = payload.get("stats") if isinstance(payload, dict) else {}
        signature = clean_text((user or {}).get("signature"))
        email = extract_email(signature) or extract_email(json.dumps(payload, ensure_ascii=False))
        ...
        summary_items.append(
            {
                ...
                "before": before,
                "api_extract": {
                    "handle": (user or {}).get("uniqueId") or handle,
                    "name": (user or {}).get("nickname"),
                    "followerCount": (stats or {}).get("followerCount") or (user or {}).get("followerCount"),
                    "email": email,
                    "bio_preview": signature[:200],
                    "links_count": len(((user or {}).get("bioLinks") or [])) or (1 if ((user or {}).get("bioLink") or {}).get("link") else 0),
                },
            }
        )
