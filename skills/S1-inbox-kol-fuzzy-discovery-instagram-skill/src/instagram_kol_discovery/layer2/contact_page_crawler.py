...
class ContactPageCrawler:
    ...
    def find_email(self, links_value: Any) -> ContactCrawlResult:
        ...
        queue = list(urls[: self.max_urls_per_creator])
        last_error = ""
        while queue and len(checked) < self.max_urls_per_creator:
            url = queue.pop(0)
            if url in seen_urls:
                continue
            seen_urls.add(url)
            try:
                ...
                for next_url in _discover_follow_up_urls(response.url, response.text):
                    if next_url not in seen_urls and next_url not in queue and len(queue) + len(checked) < self.max_urls_per_creator:
...
def _normalize_url(value: str) -> str:
    ...
    if text.startswith("//"):
        text = f"https:{text}"
    parsed_existing = urlparse(text)
    if parsed_existing.scheme and parsed_existing.scheme not in {"http", "https"}:
        return ""
...
def _discover_follow_up_urls(base_url: str, html_text: str, *, max_urls: int = 6) -> list[str]:
    host = urlparse(base_url).netloc.lower()
    candidates: list[str] = []

    for raw_href in HREF_RE.findall(html_text[:500_000]):
        normalized = _normalize_embedded_url(base_url, raw_href)
        if not normalized:
            continue
        normalized_host = urlparse(normalized).netloc.lower()
        if _is_skippable_host(normalized_host) and not _is_link_page_host(normalized_host):
            continue
        if normalized_host == host or _is_link_page_host(host) or _is_link_page_host(normalized_host):
            _append_unique(candidates, normalized)

    for raw_url in URLISH_RE.findall(html_text[:500_000]):
        normalized = _normalize_embedded_url(base_url, raw_url)
        if not normalized:
            continue
        normalized_host = urlparse(normalized).netloc.lower()
        if _is_skippable_host(normalized_host) and not _is_link_page_host(normalized_host):
    ...
    follow_ups: list[str] = []
    for candidate in candidates:
        _append_unique(follow_ups, candidate)
        candidate_host = urlparse(candidate).netloc.lower()
        if not _is_link_page_host(candidate_host):
            for path in CONTACT_PATHS:
                _append_unique(follow_ups, urljoin(candidate, path))
        if len(follow_ups) >= max_urls:
            break
    return follow_ups[:max_urls]


def _normalize_embedded_url(base_url: str, value: str) -> str:
    text = safe_text(value).strip("<>()[]{}\"'")
