...
from instagram_kol_discovery.layer2.discovery_master import persist_discovery_master, route_candidates_with_discovery_master
from instagram_kol_discovery.layer2.instagram_enrichment import InstagramProfileEnricher


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Instagram KOL Layer2 enrichment from existing L1 candidates.")
    parser.add_argument("--input-csv", required=True, help="Path to Instagram L1 candidate CSV.")
    ...
    parser.add_argument("--followers-gate", type=int, default=2000, help="Followers gate for L3 eligibility.")
    ...
    enricher = InstagramProfileEnricher(
    ...
    enriched_csv = output_dir / f"instagram_kol_L2_enriched{batch_suffix}.csv"
    shortlist_csv = output_dir / f"instagram_kol_L2_shortlist{batch_suffix}.csv"
    runlog_json = output_dir / f"instagram_kol_L2_runlog{batch_suffix}.json"
    audit_csv = output_dir / f"instagram_kol_L2_master_audit{batch_suffix}.csv"
    audit_json = output_dir / f"instagram_kol_L2_master_audit{batch_suffix}.json"

    shortlist = [row for row in enriched_rows if row.get("recommended_action") in {"keep", "review"}]
    write_csv(enriched_csv, enriched_rows, preferred_fields=INSTAGRAM_L2_REQUIRED_FIELDS + INSTAGRAM_L2_FLEX_FIELDS)
