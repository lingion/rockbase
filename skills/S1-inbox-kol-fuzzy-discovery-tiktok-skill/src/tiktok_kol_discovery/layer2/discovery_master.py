...
from tiktok_kol_discovery.io_utils import read_csv, safe_text, skill_docs_dir, write_csv
...
def discovery_master_path() -> Path:
    return skill_docs_dir() / "tiktok_kol_discovery_master.csv"
...
def _project_key_from_batch(batch_name: str) -> str:
    text = safe_text(batch_name)
    if "_batch_" in text:
        return text.split("_batch_", 1)[0]
    return text


def _normalized_status(row: dict[str, str] | None) -> str:
    return safe_text((row or {}).get("master_status")).lower() or "active"


def _existing_project_key(row: dict[str, str] | None) -> str:
    row = row or {}
    return safe_text(row.get("last_seen_project") or row.get("first_seen_project") or _project_key_from_batch(safe_text(row.get("last_seen_batch") or row.get("first_seen_batch"))))
...
def route_candidates_with_discovery_master(
    ...
    *,
    ...
    project_key = _project_key_from_batch(batch_name)
    ...
    for candidate in candidates:
        ...
        route_status = "new_candidate"
        reason = "not_in_master"
        if master_row:
            master_status = _normalized_status(master_row)
            same_batch = batch_name in {
                safe_text(master_row.get("first_seen_batch")),
                safe_text(master_row.get("last_seen_batch")),
            }
            same_project = project_key and project_key == _existing_project_key(master_row)
            if master_status in {"suppressed", "do_not_contact", "blocked"}:
                route_status = "blocked_by_master"
                reason = f"hard_suppression:{master_status}"
            elif same_batch:
                route_status = "same_batch_rerun"
                reason = "allow_rerun_same_batch"
            elif same_project:
                route_status = "same_project_seen"
                reason = "allow_seen_in_same_project"
            else:
                route_status = "historical_seen_soft"
                reason = "allow_seen_in_history"
        audit_rows.append(
            {
        ...
        if route_status == "blocked_by_master":
    ...
    row.update(
        {
            ...
            "profile_url": safe_text(enriched_row.get("profile_url")) or f"https://www.tiktok.com/@{safe_text(candidate.get('creator_handle'))}",
            "first_seen_date": safe_text(existing.get("first_seen_date")) or run_date,
            "last_seen_date": run_date,
            "first_seen_project": safe_text(existing.get("first_seen_project")) or _project_key_from_batch(batch_name),
            "last_seen_project": _project_key_from_batch(batch_name),
            ...
            "master_status": safe_text(existing.get("master_status")) or "active",
        }
    )
    if enriched_row:
        row.update(
