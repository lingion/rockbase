...
from youtube_kol_discovery.layer2.scrapecreators_client import normalize_links
...
def extract_emails_from_text(text: str) -> list[str]:
    ...
    for candidate in candidates:
        ...
        if not _is_plausible_email(email_value):
            continue
        if email_value.lower() not in {item.lower() for item in out}:
