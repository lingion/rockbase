...
from youtube_kol_discovery.io_utils import backup_csv_if_exists, read_csv, safe_text, skill_docs_dir, write_csv
...
def discovery_master_path() -> Path:
    return skill_docs_dir() / "youtube_kol_discovery_master.csv"
    ...
    row.update(
        {
            ...
            "profile_url": safe_text(enriched_row.get("profile_url"))
            or f"https://www.youtube.com/channel/{safe_text(candidate.get('channel_id'))}",
