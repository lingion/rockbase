...
- 本 skill 由 `S3-codex-plugin-reply` 在 `ocr_queue`、价格证据缺口或完整 thread recovery 需要时调用。
- 不作为日常 ReplyOps 的独立启动入口；日常运行从 `S3-codex-plugin-reply` 的 WF-1 开始。
- 默认 Gmail 账户为 `<YOUR_ACCOUNT_EMAIL>`，正式主表为 `Agency/list-master/【S3 ReplyOps】Corestar-Replied-KOL_V3.csv`。
...
`{PROJECT_ROOT}/workbench/rockbase-gmail-reply-recovery-v3/`。`PROJECT_ROOT` 指当前项目根，即同时包含 `Agency/` 与 `workbench/` 的目录；不得再把本机绝对路径写入新脚本或说明。
...
- 修改 LLM input pack、OCR 证据映射或文件命名逻辑后，先运行：`PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s .agent/skills/S3-ag-reply-recovery-sync-v3/tests -p 'test_*.py'`。
- 该测试只使用临时 CSV fixture；不访问 Gmail、不运行 OCR、不写主表。
