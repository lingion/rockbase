...
DEFAULT_HUB_ROOT = Path("${ROCKBASE_HOME}/Library/Mobile Documents/iCloud~md~obsidian/Documents/My Vault/400 🔴 Project/🔴 420 Social Agency/workbench/reply-recovery-hub")
...
S2_MASTER_FILENAMES = [
    ...
    (
        "TikTok_2026-05-13",
        "${ROCKBASE_HOME}/Library/Mobile Documents/iCloud~md~obsidian/Documents/My Vault/400 🔴 Project/🔴 420 Social Agency/.agent/skills/S1-inbox-kol-fuzzy-discovery-tiktok-skill/deliverables/2026-05-13/【S2_cold】2026-05-13_tiktok_kol_S2_merged_final.csv",
    ),
    (
        "YouTube_2026-05-13",
        "${ROCKBASE_HOME}/Library/Mobile Documents/iCloud~md~obsidian/Documents/My Vault/400 🔴 Project/🔴 420 Social Agency/.agent/skills/S1-inbox-kol-fuzzy-discovery-youtube-skill/deliverables/2026-05-13/【S2_cold】2026-05-13_youtube_kol_S2_merged_final.csv",
    ),
]
...
def execute_with_retry(request, *, attempts: int = 4, base_sleep: float = 1.5):
    ...
    for attempt in range(1, attempts + 1):
        ...
        except (IncompleteRead, HttpError, ssl.SSLError) as exc:
            retryable = isinstance(exc, (IncompleteRead, ssl.SSLError))
...
def build_outbound_map(messages: List[dict], our_email: str) -> Tuple[str, Dict[str, str]]:
    ...
    mapping = {}
    parts = []
    for idx, msg in enumerate(outbound_msgs[:3], start=1):
        mid = msg.get("id", "")
...
def update_wave_specific_fields(row: Dict[str, str], matched_wave: str, latest_inbound: Dict[str, str], next_action: str) -> None:
    ...
    row[f"{prefix}_Reply_At"] = latest_inbound.get("reply_at", "")
    row[f"{prefix}_Reply_Summary"] = compact_preview(latest_inbound.get("body_text", ""))
    row[f"{prefix}_Reply_Category"] = f"{matched_wave}_reply"
    row[f"{prefix}_Reply_Next_Action"] = next_action
    row[f"{prefix}_Reply"] = (latest_inbound.get("body_text", "") or "").strip()
...
def load_s2_candidates(list_master_dir: Path) -> Dict[str, List[Dict[str, str]]]:
    ...
    for source_tag, filename in S2_MASTER_FILENAMES:
        ...
        if not path.is_absolute():
            path = (list_master_dir / filename).resolve()
        if not path.exists():
            continue
...
def main() -> None:
    ...
    if checkpoint.get("last_success_reply_at"):
        ...
        if row is None:
            ...
            if template_row is None:
                ...
                if bootstrap_row is None:
                    ...
                    if latest_inbound["from_email"] == "<YOUR_ACCOUNT_EMAIL>" or "delivery status notification" in subject_norm:
                        issue_type = "bounce_excluded"
                        details = f"bootstrap_match={bootstrap_match} | subject={subject_norm}"
                    audit_rows.append(
                        {
