...
def run_dual_image_ocr(image_path: Path, txt_path: Path) -> Tuple[str, Dict[str, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="reply_image_ocr_", dir=str(temp_parent)) as temp_dir:
        temp_root = Path(temp_dir)
        processed_path = build_ocr_variant(image_path, temp_root / f"{image_path.stem}_prep.png")
        candidates = []
        errors = []
        for label, source_path, psm in [
            ("ocr_raw_psm6", image_path, 6),
            ("ocr_preprocessed_psm11", processed_path, 11),
        ]:
            try:
        ...
        if not candidates:
            raise RuntimeError("all OCR variants failed: " + " | ".join(errors))
...
def run_pdf_ocr(pdf_path: Path, txt_path: Path) -> Tuple[str, Dict[str, str]]:
    temp_parent = txt_path.parent if txt_path.parent.exists() else pdf_path.parent
    with tempfile.TemporaryDirectory(prefix="reply_pdf_ocr_", dir=str(temp_parent)) as temp_dir:
...
def main() -> None:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        try:
            ...
            if lower_name.endswith(".pdf") or mime_type == "application/pdf":
                try:
                    ...
                    if shutil.which("pdftoppm") and shutil.which("tesseract"):
                        ocr_text, ocr_meta = run_pdf_ocr(raw_path, text_path)
            ...
            elif mime_type.startswith("image/") or lower_name.endswith((".png", ".jpg", ".jpeg", ".webp")):
